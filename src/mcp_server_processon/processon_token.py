...
# 用户token
PROCESSON_API_TOKEN = os.getenv('PROCESSON_API_TOKEN')
...
def check_api_key() -> str:
    """
    检查 PROCESSON_API_TOKEN 是否已设置
    :return: API Token
    """
    if not PROCESSON_API_TOKEN:
        raise ValueError("PROCESSON_API_TOKEN 环境变量未设置")
    return PROCESSON_API_TOKEN
...
async def check() -> str:
    """查询用户当前配置token"""
    return os.getenv('PROCESSON_API_TOKEN')
...
async def createProcessOnMind(
    ...
    try:
        ...
        url = f"{API_BASE}/api/personal/import"
        request_data = {
            'file_type': '',
            'folder': '6870b1b248b15d3ee28cb569',  # 目标文件夹ID
            'category': 'mind',
